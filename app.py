...
st.title("Public Sentiment of Enterprise Artificial Intelligence")
st.markdown("Let's explore how the public feels about Artificial Intelligence. Data pulled from Reddit and NewsAPI.")
st.markdown(
    """
    <div class="summary-card">
    This dashboard compares public AI sentiment across major enterprise AI companies
    using Reddit and NewsAPI records. It highlights which companies receive the
    strongest risk-weighted sentiment, how sentiment changes over time, and how a
    simple neural network forecast compares against a historical baseline.
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.subheader("Which AI Companies Draw the Strongest Risk Sentiment?")
...
if weekly_df is None:
    ...
    if "risk_sentiment_score" in weekly_display.columns:
        weekly_series.append(
            {
                "actual_label": "Weekly risk sentiment",
                "actual_column": "risk_sentiment_score",
                "rolling_label": "4-week risk sentiment average",
    ...
    if sentiment_column:
        weekly_series.append(
            {
                "actual_label": "Weekly average sentiment",
                "actual_column": sentiment_column,
                "rolling_label": "4-week sentiment average",
        ...
        for index, series_config in enumerate(weekly_series):
            ...
            weekly_chart.add_trace(
                go.Scatter(
                    ...
                    mode="lines+markers",
                    name=series_config["actual_label"],
                    visible="legendonly",
...
st.subheader("Forecasting Experiment: Baseline vs Neural Network")
        ...
        for index, (label, column) in enumerate(available_forecasts.items()):
            forecast_chart.add_trace(
                go.Scatter(
                    ...
                    mode="lines+markers",
                    name=label,
                    line={"color": LINE_COLORS[index % len(LINE_COLORS)]},
                )
            )
        forecast_chart.update_layout(
            title="Actual Sentiment vs Historical Baseline vs Feedforward Neural Network",
            xaxis_title="Week",
...
st.markdown(
    """
    - Collected AI-related public discussion from Reddit and NewsAPI.
    - Applied sentiment scoring using VADER and text-derived features.
    - Labeled risk-related discussion using keyword-based risk indicators.
    - Aggregated records into weekly company-level sentiment trends.
    - Compared a feedforward neural network forecast against a historical mean baseline.
    - Built a Streamlit dashboard to explore company sentiment, discussion volume, weekly trends, and forecasting results.
    """
)
